    ...
    for b in budgets:
        ...
        for budget_year in budget_grouping_on_parent:
            for monthly_row in monthly_distribution.percentages:
                ...
                # Using dictionary-style access with square brackets
                data[0][budget_key_name] = f"{((budget_grouping_on_parent[budget_year].get('Revenue from ordinary line of Business', 0) * monthly_row.percentage_allocation)/100):.3f}"
                data[1][budget_key_name] = f"{((budget_grouping_on_parent[budget_year].get('Cost of Sales', 0) * monthly_row.percentage_allocation)/100):.3f}"
                data[2][budget_key_name] = None
                
                # Update budget values for data[3] and data[5]
                data[3][budget_key_name] = float(data[0][budget_key_name]) - float(data[1][budget_key_name])
                
                # Fixed key access and syntax in the following calculations
...
def calculate_financial_metrics(income, expense, filters):
    ...
    months = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec']
    original_start_year = int(filters['from_fiscal_year'])
    end_year = int(filters['to_fiscal_year'])

    # Loop through each year first
    for year in range(original_start_year, end_year + 1):
        # Then loop through each month for that year
        for month in months:
            key = f"{month}_{year}"
            
            # Use get() with a default value to avoid KeyError
            finance_cost = float(finance_cost_row.get(key, 0))
            depreciation = float(depreciation_row.get(key, 0))
            
            # Check if the key exists in filtered_data before updating it
            if key in filtered_data[7]:
                filtered_data[7][key] -= (finance_cost + depreciation)
